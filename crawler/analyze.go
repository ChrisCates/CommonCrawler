...
func (c *crawler) AnalyzeFile(filePath string, path string) error {
	...
	go c.Analyze("ninja", file, matched)

	for m := range matched {
		color.Blue("\t  Found %d matches in this wet file...", m.Matches)

		result := fmt.Sprintf("%sMATCHES: %d\n\n", m.WarcData, m.Matches)
		_, _ = data.WriteString(result)
	}
...
func (c *crawler) Analyze(searchFor string, in io.Reader, matched chan MatchedWarc) {
	defer close(matched)
